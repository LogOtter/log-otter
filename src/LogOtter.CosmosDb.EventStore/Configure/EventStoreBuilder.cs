...
internal class EventStoreBuilder : IEventStoreBuilder
{
        ...
        JsonSerializerSettings? jsonSerializerSettings = null) where TBaseEvent : class, IEvent<TSnapshot> where TSnapshot : class, ISnapshot, new()
    {
        ...
        _cosmosDbBuilder.AddContainer<TBaseEvent>(eventContainerName, "/streamId", defaultTimeToLive: -1);
    ...
    public IEventStoreBuilder AddCatchupSubscription<TBaseEvent, TCatchupSubscriptionHandler>(string projectorName)
        where TCatchupSubscriptionHandler : class, ICatchupSubscription<TBaseEvent>
    {
        Services.AddSingleton<TCatchupSubscriptionHandler>();

        _cosmosDbBuilder
            .AddChangeFeedProcessor<CosmosDbStorageEvent, TBaseEvent, Event<TBaseEvent>, EventConverter<TBaseEvent>,
                CatchupSubscriptionChangeFeedProcessor<TBaseEvent, TCatchupSubscriptionHandler>>(projectorName);

        return this;
    }

    private static IReadOnlyCollection<Type> GetEventsOfTypeFromSameAssembly<TBaseEvent>() where TBaseEvent : class
    {
