...
public class ConcurrencyTests
{
    ...
    public async Task CanSimulateAConcurrencyException()
    {
        ...
        Func<Task> mustPassETag = () => sut.UpsertItemAsync(
            new TestClass { Id = "MyId", PartitionKey = "APartition", MyValue = "Value2" },
            new PartitionKey("APartition"));
        (await mustPassETag.Should().ThrowAsync<InvalidOperationException>()).Which.Message.Should()
        ...
        Func<Task> shouldPreConditionFail = () => sut.UpsertItemAsync(
            new TestClass { Id = "MyId", PartitionKey = "APartition", MyValue = "Value2" },
            new PartitionKey("APartition"),
