...
internal class ContainerData
{
    ...
    private ConcurrentDictionary<PartitionKey, ConcurrentDictionary<string, ContainerItem>> _data;
    ...
    public async Task<Response> UpsertItem(string json, PartitionKey partitionKey, ItemRequestOptions requestOptions,
    ...
    public ContainerDataSnapshot CreateSnapshot()
    {
        ...
        foreach (var (partitionKey, items) in _data)
        {
            ...
            foreach (var (id, containerItem) in items)
            {
                json.WriteStartObject();
