...
///     Note: This is only required on the immediate child of the <see cref="OptionallyPatched{T}" /> parameter, nested children can use attributes as
///     usual
/// </summary>
public record BasePatchRequest : IValidatableObject
{
        ...
        GetPropertiesWithValidationAttributes(Type type)
    {
            ...
            GetValidationAttributesFromPropertiesOrConstructorParams(Type t)
        {
            ...
            {
                if (!property.PropertyType.GetInterfaces().Contains(typeof(IOptionallyPatched)))
                {
                    continue;
                }
