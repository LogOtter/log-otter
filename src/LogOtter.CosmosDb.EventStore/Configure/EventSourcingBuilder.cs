...
public class EventSourcingBuilder
{
    ...
    {
        var config = new EventSourceConfiguration<TBaseEvent>(typeof(SnapshotProjectionCatchupSubscription<,>));
    ...
    public EventSourcingBuilder AddEventualEvent<TBaseEvent>(string containerName, Action<EventSourceConfiguration<TBaseEvent>>? configure = null)
        where TBaseEvent : class
    {
        var config = new EventSourceConfiguration<TBaseEvent>(typeof(EventualSnapshotProjectionCatchupSubscription<,>));
        configure?.Invoke(config);

        var changeFeedProcessorsMetadata = new List<ChangeFeedProcessorMetadata>();

        var catchupSubscriptionChangeFeedProcessor = typeof(EventualCatchupSubscriptionChangeFeedProcessor<,>);
        foreach (var catchUpSubscription in config.CatchUpSubscriptions)
        {
            Services.AddSingleton(catchUpSubscription.HandlerType);
            ...
            changeFeedProcessorsMetadata.Add(
                new ChangeFeedProcessorMetadata(
                    typeof(CosmosDbEventualStorageEvent),
                    typeof(TBaseEvent),
                    typeof(EventualEvent<TBaseEvent>),
                    typeof(EventualEventConverter<TBaseEvent>),
                    specificCatchUpSubscriptionType,
                    catchUpSubscription.ProjectorName
                )
            );
        ...
        var eventRepository = typeof(EventualEventRepository<,>);
        var snapshotRepository = typeof(EventualSnapshotRepository<,>);
        foreach (var projection in config.Projections)
        {
            Services.AddSingleton(eventRepository.MakeGenericType(typeof(TBaseEvent), projection.ProjectionType));
        ...
        Services.AddSingleton(sp =>
        {
            ...
            return new EventualEventStore<TBaseEvent>(cosmosContainer.Container, feedIteratorFactory, simpleSerializationTypeMap);
        });
